...
class WaveRNN(nn.Module):
    ...
    def generate(self, mels, batched, target, overlap, mu_law, apply_preemphasis, progress_callback=None):
        ...
        with torch.no_grad():
            ...
            mels = folding.pad_tensor(mels.transpose(1, 2), pad=self.pad, side='both')
            mels, aux = self.upsample(mels.transpose(1, 2))

            if batched:
                mels = folding.fold_with_overlap(mels, target, overlap)
                aux = folding.fold_with_overlap(aux, target, overlap)
        ...
        if batched:
            output = folding.xfade_and_unfold(output, target, overlap)
        else:
            output = output[0]

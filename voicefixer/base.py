...
class VoiceFixer(nn.Module):
    def __init__(self, analysis_model_path="", synthesis_model_path=""):
        super(VoiceFixer, self).__init__()
        self.analysis_module_ckpt = analysis_model_path
        self.synthesis_module_ckpt = synthesis_model_path

        if not os.path.exists(self.analysis_module_ckpt):
            raise RuntimeError("Error: The checkpoint for analysis module could not be found!")
        if not os.path.exists(self.synthesis_module_ckpt):
            raise RuntimeError("Error: The checkpoint for synthesis module could not be found!")

        self._model = voicefixer_fe(channels=2, sample_rate=44100, vocoder_path=synthesis_model_path)
    ...
    @torch.no_grad()
    def restore_inmem(self, wav_10k, cuda=False, mode=0, your_vocoder_func=None, do_vocode=True):
